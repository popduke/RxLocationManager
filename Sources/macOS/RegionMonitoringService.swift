...
public protocol RegionMonitoringService: RegionMonitoringServiceConfigurable{
    ...
    var regionState: Observable<(CLRegion, CLRegionState)>{get}
    var error: Observable<(CLRegion?, NSError)>{get}
    
    func requestState(for region: CLRegion) -> RegionMonitoringService
}
...
class DefaultRegionMonitoringService: RegionMonitoringService{
    ...
    private var regionStateObservers = [(id:Int, observer: AnyObserver<(CLRegion, CLRegionState)>)]()
    ...
    var regionState: Observable<(CLRegion, CLRegionState)>{
        get{
            return Observable.create{
                observer in
                var ownerService:DefaultRegionMonitoringService! = self
                let id = nextId()
                ownerService.regionStateObservers.append((id, observer))
                return AnonymousDisposable{
                    ownerService.regionStateObservers.removeAtIndex(ownerService.regionStateObservers.indexOf{$0.id == id}!)
                    ownerService = nil
                }
            }
        }
